...
- **Zwei Regale für einen Keller** entstehen, wenn auf einem zweiten Gerät „Regal anlegen“ getippt wird, bevor das erste eingetroffen ist. `Rack.preferred` nimmt deshalb immer das **älteste** des aktiven Kellers, der Leerzustand weist auf den Abgleich hin, und `Rack.mergeDuplicates` führt sie zusammen: Fächer wandern zum ältesten, bei belegter Position wird die Flasche nur aus dem Regal genommen. Der Bestand am Wein bleibt in jedem Fall unangetastet. Waren die Regale **unterschiedlich gross**, wächst das behaltene beim Zusammenführen mit; sonst läge ein übernommenes Fach ausserhalb des Rasters und die Flasche wäre zwar verortet, aber nirgends zu sehen.
...
- **Code am Keller, nicht am Gerät** (`Cellar.partyCodeHash`, gesetzt über `PartyLock`): 4–8 Ziffern, gespeichert als **gesalzener** SHA256-Hash (Salz = `Cellar.uuid`, damit derselbe Code in zwei Kellern nicht denselben Hash ergibt). So gilt er auf iPhone **und** iPad und – bei geteiltem Keller – auch bei der Partnerin. Der erste Entwurf legte ihn in die Keychain; Richard hat am 10.9.2026 zu Recht angemerkt, dass er damit gerätegebunden wäre. Der alte Keychain-Eintrag `party.passcode` wird beim Setzen aufgeräumt. Kein Schutz gegen Angreifer (wer die Datenbank hat, hat ohnehin alle Weine), sondern gegen Gäste mit dem Gerät in der Hand. **Face ID ist der zweite Weg hinaus** (`INFOPLIST_KEY_NSFaceIDUsageDescription`) – ohne den wäre man bei vergessenem Code in der eigenen App gefangen, weil ein Neustart bewusst nicht hilft. Ändern nur mit dem bisherigen Code.
...
- **Historie** (`PartyWin`, Core Data, am Keller): Name, Untertitel und ein verkleinertes Etikett stehen als **Kopie** drin, nicht als Beziehung – die Flasche wird getrunken und irgendwann gelöscht, „Silvester 2026: der Barolo“ soll bleiben. `wineUUID` nur als Zusatz. Weil sie am Keller hängt, wandert sie mit der Freigabe mit; `PartyHistoryView` filtert deshalb wie das Regal auf `Cellar.current` – sonst stünden beim Gast zwei Historien durcheinander. **Neue Entität und neues Cellar-Feld → vor dem nächsten TestFlight „Deploy Schema Changes“ in der CloudKit-Konsole.**
