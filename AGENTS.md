...
- Kamera: `DocumentScannerView` (VisionKit `VNDocumentCameraViewController`) erkennt das Etikett live, schneidet zu und begradigt. Solche Bilder kommen mit `ScanImage.isPreCropped = true` an, der Cropper überspringt dann Rechteck-/Textzuschnitt. Nicht im Simulator verfügbar (`isSupported`), dort nur Fotoauswahl.
- **Bedienung**: kein globaler Scan-Knopf. Die Felder „Vorderseite“ und „Rückseite“ in `LabelScanView` sind je eine antippbare Fläche, die den Dokumentenscanner für **diese** Seite öffnet (`LabelScanViewModel.Side`, `scanningSide` als `.fullScreenCover(item:)`). Seite 1 des Scans geht an die angetippte Seite, eine zweite Seite füllt die andere nur, wenn sie noch leer ist (`applyScannedPages(_:to:)`). „Aus Fotos“ steht unter jeder Fläche.
